...
void RenderScene()
{
	...
	D3DXMATRIXA16 matTorusWorld;
	{
		...
		if (gRotationY > 2 * PI)  gRotationY -= 2 * PI;
	...
	gpD3DDevice->Clear(0, NULL, (D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER), 0xFFFFFF, 1.0f, 0);
...
bool LoadAssets()
{
	...
	if (!gpCreateShadowShader) return false;

	gpDisc = LoadModel("Disc.x");
	if (!gpDisc) return false;
