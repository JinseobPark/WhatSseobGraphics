...
void RenderScene()
{
	...
	gpENVShader->SetMatrix("gWorldMatrix", &matWorld);
	gpENVShader->SetMatrix("gMVP", &matWorldViewProjection);

	gpENVShader->SetVector("gWorldLight", &gWorldLightPosition);
	gpENVShader->SetVector("gWorldCameraPosition", &gWorldCameraPosition);

	gpENVShader->SetVector("gLightColor", &gLightColor);
	gpENVShader->SetTexture("DiffuseMap_Tex", gpStoneDM);
	gpENVShader->SetTexture("SpecularMap_Tex", gpStoneSM);
	gpENVShader->SetTexture("NormalMap_Tex", gpStoneNM);
	gpENVShader->SetTexture("EnvironmentMap_Tex", gpSnowENV);


	UINT numPasses = 0;
	gpENVShader->Begin(&numPasses, NULL);
	{
		for (UINT i = 0; i < numPasses; ++i)
		{
			gpENVShader->BeginPass(i);
			{
				//draw
	...
	if (gPostProcessIndex == 1) effectToUse = gpGrayScale;
	else if (gPostProcessIndex == 2) effectToUse = gpSepia;
	else if (gPostProcessIndex == 3) effectToUse = gpEdgeDetection;
	else if (gPostProcessIndex == 4) effectToUse = gpEmboss;
...
bool LoadAssets()
{
	...
	if (!gpStoneDM) return false;
	gpStoneSM = LoadTexture("fieldstone_SM.tga");
	if (!gpStoneSM) return false;
	gpStoneNM = LoadTexture("fieldstone_NM.tga");
	if (!gpStoneNM) return false;
	D3DXCreateCubeTextureFromFile(gpD3DDevice, "Snow_ENV.dds", &gpSnowENV);
	if (!gpSnowENV) return false;
	// 쉐이더 로딩
	gpENVShader = LoadShader("EnvironmentMapping.fx");
	if (!gpENVShader)
		return false;
