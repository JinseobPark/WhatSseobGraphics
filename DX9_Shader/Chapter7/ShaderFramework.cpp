...
void RenderScene()
{
	...
	gpLightShader->SetMatrix("gMVP", &matWorldViewProjection);

	gpLightShader->SetVector("gWorldLight", &gWorldLightPosition);
	gpLightShader->SetVector("gWorldCameraPosition", &gWorldCameraPosition);
	...
	gpLightShader->SetTexture("NormalMap_Tex", gpStoneNM);
...
bool LoadAssets()
{
	...
	// 쉐이더 로딩
	gpLightShader = LoadShader("NormalMapping.fx");
	if (!gpLightShader)
		return false;
