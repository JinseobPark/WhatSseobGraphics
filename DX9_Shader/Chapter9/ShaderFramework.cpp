...
// 쉐이더
LPD3DXEFFECT gpLightShader = NULL;
...
void RenderScene()
{
	...
	gpLightShader->SetMatrix("gWorldMatrix", &matWorld);
	gpLightShader->SetMatrix("gViewMatrix", &matView);
	gpLightShader->SetMatrix("gProjectionMatrix", &matProjection);
	//gpLightShader->SetFloatArray("gWorldLight", gWorldLightPosition, 4);
	gpLightShader->SetVector("gWorldLight", &gWorldLightPosition);

	gpLightShader->SetVector("gLightColor", &gLightColor);
	gpLightShader->SetTexture("DiffuseMap_Tex", gpStoneDM);
	gpLightShader->SetTexture("SpecularMap_Tex", gpStoneSM);

	gpLightShader->SetFloat("gWaveHeight",3 );
	gpLightShader->SetFloat("gSpeed",2 );
	gpLightShader->SetFloat("gWaveFrequency", 10);
	gpLightShader->SetFloat("gUVSpeed", 0.25f);
	gpLightShader->SetFloat("gTime", tick/1000.0f);

	UINT numPasses = 0;
	gpLightShader->Begin(&numPasses, NULL);
	{
		for (UINT i = 0; i < numPasses; ++i)
		{
			gpLightShader->BeginPass(i);
			{
				//draw
				gpSphere->DrawSubset(0);
			}
			gpLightShader->EndPass();
		}
		gpLightShader->End();
	}
...
bool LoadAssets()
{
	...
	// 쉐이더 로딩
	gpLightShader = LoadShader("UVAnimation.fx");
	if (!gpLightShader)
		return false;
